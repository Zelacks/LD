...
namespace Zenject
{
    ...
    public class GameObjectContext : RunnableContext
    {
        ...
        public override DiContainer Container { get { return _container; } }
        ...
        void InstallBindings( List< MonoBehaviour > injectableMonoBehaviours )
        {
            ...
            if ( _kernel == null )
            {
                _container.Bind< MonoKernel >( ).To< DefaultGameObjectKernel >( ).FromNewComponentOn( this.gameObject ).AsSingle( ).NonLazy( );
            }
            else
            {
